...
const DetailLomba = async ({ params }) => {
  ...
  const embedVideoUrl = videourl
    ...
    : "";
  return (
    <div className="w-full lg:max-w-[834px] min-h-screen py-20 flex flex-col">
      <ScrollBar />
      <ButtonBack />
      <h2
        className="text-3xl lg:text-5xl py-3 font-semibold"
      ...
      <div className="flex justify-between items-center">
        <p className="text-xs lg:text-base">
          Published On <span className="font-semibold">{formattedDate}</span>
        </p>
        ...
        <div className="tags-wrapper flex justify-center gap-2">
          ...
          <ul className="flex gap-2">
            {kategori.map((item, idx) => (
              <li
                key={idx}
                className="border-[1px] border-solid border-gray-900 px-3 py-1 rounded-full text-xs"
              >
                {item}
              </li>
            ))}
